...
public class AssignClassServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Inside Assign Class method");
		String subject=request.getParameter("name");
		String course=request.getParameter("class");
		String section=request.getParameter("section");
		try {
			Statement statement = connection.createStatement();
			int result = statement.executeUpdate("update subjects set class = '" + course + "',section='"+section+"' where name='" + subject + "'");
			PrintWriter out = response.getWriter();
			if (result > 0) {
				out.println("<html>");
				out.println("<head>");
				out.println("<h1 style=text-align:center>Class Assigned</h1>");
				out.println("</head>");
				...
				out.println("<h1>Error While Assigning Class</h1>");
			}
